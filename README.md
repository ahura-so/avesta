...
Avesta encompasses and fully centers itself around sophisticated architectural ideas such as **domain-driven design**, **command-query responsibility segregation**, and **event sourcing** — the famous *DDD-CQRS-ES* trifecta — as well as **service-oriented** and **event-driven** architecture. At the code level, Avesta's design uniformly adheres to [the hexagonal architecture](https://en.wikipedia.org/wiki/Hexagonal_architecture_(software)) (a.k.a. ports & adapters, onion architecture, clean architecture — all the same). The most important high-level ideals Avesta aims to facilitate are **code correctness**, **horizontal scalability**, and **speed/performance**, in that order.

From a technical standpoint, Avesta is exclusively built on top of source-available software; its "opinions" in terms of infrastructure, also known as "stack", are the following trio:
...
NATS is Avesta's single most important piece of infrastructure. It alone powers and fulfills most of its infrastructure needs. I claim that it is the greatest modern pub/sub software out there.
...
- **End-to-end type-safety**: Across and within service boundaries — an unforgiving, robust type system with Roslyn analyzers sprinkled on top, catching and eliminating entire classes of bugs at compile-time <sup>[Tell me more](https://avestic.dev/docs/horizontal-scalability)</sup>
...
- Causality-aware timestamping mechanism using [**hybrid logical clocks (HLC)**](https://cse.buffalo.edu/tech-reports/2014-04.pdf), making the system resilient against wall-clock skew and drift, and guaranteeing correct, happens-before-reflecting timestamps regardless of physical clock quirks.
